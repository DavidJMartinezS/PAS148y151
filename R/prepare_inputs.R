...
#' @export
#'
prepare_bd_flora <- function(BD, rodales, PAS, cut_by_rod, include_fp = F) {
  ...
  (if (inherits(BD, "data.frame")) BD else openxlsx2::read_xlsx(BD)) %>%
    ...
    {if (PAS %in% c(148, 149)) {
      .[] %>%
        dplyr::filter(
          if(!include_fp) !Cob_BB %>% stringi::stri_trans_tolower() %in% c(NA_character_, "fp", "---") & !N_ind %in% c(NA, 0),
          Habito %>%
            stringi::stri_trans_general("Latin-ASCII") %>%
    ...
    {if (include_fp) .[] %>% mutate_at("N_ind", replace_na, 0) else .[]} %>%
