...
#' @param bd_flora data.frame con base de datos de parcelas flristicas. Incluir si `add_parcelas` es \code{TRUE.}. ver `detail`.
#' @param bd_pcob data.frame con base de datos de parcelas de cobertura.
#' @param bd_trans data.frame con base de datos de transectos.
#' @param rodales Objeto sf con capa de rodales.
#' @param predios Objeto sf con capa de predios.
#' @param areas Objeto sf con capa de areas de corta. default \code{NULL}.
...
#' @param join_by Union de las bases de datos `parcela` o `coordenadas`.
...
#' @param tabla_attr_rodal data.frame con tabla de atributos de rodal. ver `details`.
#' @param bd_flora_2 Opcional. Base de datos de flora que contenga las parcelas utilizadas para atributar areas de corta sin parcelas directa o cercana. ver `details`.
...
#'
#' @details
#' Antes de ingresar alguna de las bases de datos, se recomienda verificar que estas contengan los campos requeridos, entre otros errores.
#' Para esto puede utilizar las funciones [check_bd_flora()], [check_bd_pcob()], [check_bd_trans()].
#' Una ayuda a preparar las bases de datos podrian ser las funciones [prepare_bd_flora()], [prepare_bd_pcob()] o [prepare_bd_trans()].
#'
#' En el parametro `portada_opts` debe ingresar una lista con los argumentos de la funcion [portada_opts()].
#'
#' El data.frame debe contener el mismo formato que el arrojado con la funcion [get_tabla_attr_rodal()].
#'
#' La `bd_flora_2` debe contener al menos los campos; "Parcela", "Especie", "Cob_BB", "Nha" y "Habito".
#'
...
apendice_2_3 <- function(
  ...
  join_by <- match.arg(join_by, choices = c("parcela", "coordenadas"))
  cov_fp <- match.arg(as.character(cov_fp), choices = c(0, 0.5, 1))
  valid_input(cov_as_range, inherit = "logical")
  valid_df(df = bd_flora, names = c('Parcela', 'UTM_E', 'UTM_N', 'Especie', 'N_ind', 'Habito', 'Cob_BB'))
  valid_input(predios, inherit = "sf", names = "N_Predio", geometry = "POLYGON")
  ...
  huso <- ifelse(sf::st_crs(predios)$epsg == 32719, "19S", "18S")

  if (!is.null(bd_pcob)) {
    if(join_by == "parcela") {
      valid_df(df = bd_pcob, names = c('Parcela', 'Especie', 'Copa_NS', 'Copa_EO'))
    } else {
      valid_df(df = bd_pcob, names = c('Parcela', 'UTM_E', 'UTM_N', 'Especie', 'Copa_NS', 'Copa_EO'))
    }
  }
  if (!is.null(bd_trans)) {
    if(join_by == "parcela") {
      valid_df(df = bd_trans, names = c('Parcela', 'Especie', 'Habito', 'Denominador', 'Cuenta'))
    } else {
      valid_df(df = bd_trans, names = c('Parcela', 'UTM_E', 'UTM_N', 'Especie', 'Habito', 'Denominador', 'Cuenta'))
    }
  }
  ...
  df_flora <- bd_flora %>% tibble::as_tibble() %>%
  ...
  if (!is.null(bd_pcob)) {
    df_pcob <- bd_pcob %>%
      ...
      dplyr::inner_join(bd_flora %>% dplyr::count(Parcela, N_Parc, UTM_E, UTM_N) %>% dplyr::select(-n)) %>%
      dplyr::mutate(Cob_arb = purrr::map2_dbl(Copa_NS, Copa_EO, cup_coverage, method = "ellipse")) %>%
      dplyr::group_by(N_Parc, UTM_E, UTM_N, Especie) %>%
      dplyr::summarise(Cob_arb = sum(Cob_arb, na.rm = T), Nha = dplyr::n() * 20) %>%
      dplyr::group_by(N_Parc) %>%
      dplyr::mutate(
        Cob_parc_p = plot_coverage(
          Cob_arb,
          percent = T,
          digits = 1,
          decimal.mark = ","
  ...
  if (PAS == 151 & !is.null(bd_trans)) {
    df_trans <- bd_trans %>%
      ...
      dplyr::inner_join(bd_flora %>% dplyr::count(Parcela, N_Parc, UTM_E, UTM_N) %>% dplyr::select(-n)) %>%
      dplyr::filter(
        !(stringi::stri_detect_regex(Especie, "Suelo.*desnudo|identificac|indeter", case_insensitive = T) | is.na(Especie)) &
          (stringi::stri_detect_regex(Habito, "arbust", case_insensitive = T) | stringi::stri_detect_regex(Habito, "sucul", case_insensitive = T))
      ) %>%
      ...
      {if (dplyr::anti_join(.[], bd_flora %>% dplyr::count(Parcela, N_Parc, UTM_E, UTM_N)) %>% nrow() > 0) {
        warning(
          ...
          dplyr::anti_join(
            .[],
            bd_flora %>% dplyr::count(Parcela, N_Parc, UTM_E, UTM_N)
          ) %>%
  ...
  if (PAS == 148) {
    ft_2 <- dplyr::bind_rows(df_flora, df_pcob) %>%
      tidyr::pivot_wider(names_from = Fuente, values_from = c(Nha, Cob_arb, Cob_parc)) %>%
      ...
      dplyr::select(
        ...
        UTM_N,
        Especie,
        dplyr::ends_with("PCob"),
        dplyr::ends_with("Flora")
      ) %>%
      `names<-`(
        c(
          "Parcela",
      ...
      flextable::merge_v(j = 1, target = c(1:3, 7, 10)) %>%
      ...
      flextable::theme_box() %>%
      {
        if (!is.null(df_pcob)) {
          flextable::bg(
            .,
            i = ~ `P.COB_Cobertura por parcela (%)` < 10,
            j = 7,
            bg = "yellow"
          ) %>%
            flextable::bg(
              i = ~ `FLORA_Cobertura por parcela (%)` < 10,
              j = 10,
              bg = "yellow"
            )
        } else {
    ...
    ft_2 <- df_flora %>%
      {if (!is.null(df_pcob)) {
        .[] %>% dplyr::left_join(
          df_pcob %>% dplyr::select(N_Parc, Especie, Cob_arb) %>% dplyr::rename(Pcob_cob = Cob_arb)
        )
      } else .[]} %>%
      {if (!is.null(df_trans)) {
        .[] %>% dplyr::left_join(df_trans %>% dplyr::select(N_Parc, Especie, Trans_cob))
      } else .[]} %>%
      dplyr::arrange(N_Parc, Especie) %>%
      `names<-`(
        c(
          ...
          "NHA\n(árb/ha)",
          "Cobertura (%)",
          {if (!is.null(df_pcob)) c("Cobertura P.Cob (%)")},
          {if (!is.null(df_trans)) c("Cobertura Trans (%)")}
        )
      ) %>%
      flextable::flextable() %>%
      flextable::separate_header(split = "_") %>%
      flextable::merge_v(j = 1, target = c(1:3)) %>%
      flextable::italic(j = 4) %>%
      flextable::autofit() %>%
      {if (cov_as_range) {
        flextable::bg(
  ...
  wb_ap2 <- openxlsx2::wb_workbook(theme = "Integral") %>%
    ...
    flexlsx::wb_add_flextable(
      sheet = "SP_Nha_y_Cobertura_Parcelas",
      ft = ft_2
    )
  ...
  wb_ap3 <- openxlsx2::wb_workbook(theme = "Integral") %>%
    ...
    flexlsx::wb_add_flextable(
      sheet = "Ubicación_Parcelas",
      ft = ft_3
    )

  return(list(Apendice_2 = wb_ap2, Apendice_3 = wb_ap3))
}

#' @rdname get_appendices
#' @export
apendice_5_PAS148 <- function(
  ...
  valid_df(df = bd_flora, names = c('Parcela', 'UTM_E', 'UTM_N', 'Especie', 'N_ind', 'Habito', 'Cob_BB'))
  valid_input(rodales, inherit = "sf", names = "Sup_ha", geometry = "POLYGON")
  valid_df(tabla_predios, names = c('N_Predio', 'Nom_Predio', 'Rol', 'Propietari', 'Comuna', 'Provincia', 'Region', 'Sup_ha'))
  valid_df(tabla_areas, names = c('N_Predio', 'N_Area', 'Clase_Uso', 'Pend_media', 'Sup_ha'))
  valid_df(tabla_attr_rodal, names = c("N_Rodal", "Tipo_fores", "Subtipo_fo", "Tipo_veg", "Tipo_attr", "Nom_attr"))
  ...
  wb_ap5 <- wb_ap5 %>%
    ...
    openxlsx2::wb_set_cell_style(
      dims = openxlsx2::wb_dims(x = tbl_predios, select = "col_names"),
      style = wb_ap5$styles_mgr$get_xf_id("header_cellxfs")
    ) %>%
    openxlsx2::wb_add_border(
      dims = openxlsx2::wb_dims(x = tbl_predios, select = "data"),
      inner_hgrid = "thin",
      inner_vgrid = "thin"
    ) %>%
  ...
  wb_ap5 <- wb_ap5 %>%
    ...
    openxlsx2::wb_set_cell_style(
      dims = openxlsx2::wb_dims(x = tbl_predios_sup, select = "col_names"),
      style = wb_ap5$styles_mgr$get_xf_id("header_cellxfs")
    ) %>%
    openxlsx2::wb_add_border(
      dims = openxlsx2::wb_dims(x = tbl_predios_sup, select = "data"),
      inner_hgrid = "thin",
      inner_vgrid = "thin"
    ) %>%
    openxlsx2::wb_add_numfmt(
      dims = openxlsx2::wb_dims(x = tbl_predios_sup, cols = 4, select = "data"),
      numfmt = "#,##0.00"
    ) %>%
  ...
  if (!is.null(carto_uso_actual)) {
    tbl_uso_actual <- carto_uso_actual %>%
      ...
      tidyr::pivot_wider(names_from = Uso_Actual, values_from = Sup_ha) %>%
  ...
  # Tabla hidrografia ----
  if (any(c("ID_Hidro", "Tipo_Dren", "Tipo_Perma", "Nombre_curso") %in% names(tabla_areas))) {
    tbl_hidro <- tabla_areas %>%
      ...
      `names<-`(
        c(
          ...
          "Cursos de agua",
          "Temporalidad",
          "Distancia al área a intervenir (m)",
          "Ancho del cauce (m)"
        )
      ) %>%
      flextable::flextable() %>%
    ...
    wb_ap5 <- wb_ap5 %>%
      ...
      flexlsx::wb_add_flextable(sheet = "Recursos_hídricos", ft = tbl_hidro)
  }
  ...
  nha_ptos <- tabla_areas %>%
    ...
    dplyr::filter(Nom_attr %>% stringi::stri_detect_regex("Parcela", case_insensitive = T)) %>%
    dplyr::mutate(nha_x_sp = purrr::map(Parcelas, nha_x_sp_fun, bd = bd_flora)) %>%
    {if (!is.null(bd_flora_2)) {
      dplyr::bind_rows(
        tabla_areas %>%
          tibble::as_tibble() %>%
          ...
          dplyr::filter(
            Nom_attr %>% stringi::stri_detect_regex("Parcela", case_insensitive = T)
          ) %>%
          dplyr::mutate(
            nha_x_sp = purrr::map(
              Parcelas,
  ...
  nha_est <- tabla_areas %>%
    dplyr::left_join(tabla_attr_rodal %>% dplyr::select(N_Rodal, Tipo_veg, Tipo_attr)) %>%
  ...
  nha_otros <- tabla_areas %>%
    ...
    dplyr::filter(Tipo_veg %in% c(setdiff(
      unique(rodales$Tipo_veg), unique(bd_flora$Tipo_veg)
    ))) %>%
  ...
  obs_fun <- function(x) {
    ...
    if (is.na(x) %>% all()) {
      ...
      paste("Se utilizaron las parcelas: ", paste(x, collapse = ", "))
    }
  }

  tabla_attr_rodal_final <- tabla_attr_rodal %>%
    dplyr::mutate(Nom_attr = dplyr::case_when(
      (!N_Rodal %in% unique(dplyr::bind_rows(nha_ptos, nha_est, nha_otros) %>% .$N_Rodal) &
         Nom_attr %>% stringi::stri_detect_regex("similar", case_insensitive = T)) ~
        "Encontrar otro método de estimación",
      .default = Nom_attr
    )) %>%
    dplyr::left_join(
      nha_otros %>%
        ...
        dplyr::bind_rows(
          parc_x_tipo %>%
            ...
            dplyr::inner_join(
              tabla_attr_rodal %>%
                dplyr::filter(Nom_attr == "Estimación por Tipo vegetacional") %>%
                dplyr::select(Tipo_veg, N_Rodal)
            ) %>%
  ...
  df_veg <- dplyr::bind_rows(nha_ptos, nha_est, nha_otros) %>%
    ...
    dplyr::left_join(tabla_areas %>% dplyr::select(N_Rodal, N_Area, Sup_ha)) %>%
    dplyr::left_join(tabla_attr_rodal %>% dplyr::select(N_Rodal, Tipo_fores, Subtipo_fo)) %>%
  ...
  ft_veg <- df_veg %>%
    ...
    flextable::flextable() %>%
    flextable::merge_v(j = 1) %>%
    flextable::merge_v(j = 2, target = c(2, 4, 7:10)) %>%
  ...
  for (i in seq_len(nrow(stf))) {
    ft_veg <- flextable::footnote(
      ft_veg,
      part = 'body',
      i = c(which(df_veg$Subtipo_fo == stf$Subtipo_fo[i])),
      j = 3,
  ...
  wb_ap5 <- wb_ap5 %>%
    ...
    openxlsx2::wb_add_data(
      sheet = "Attr_Rodal",
      x = tabla_attr_rodal_final,
      start_col = 1,
      start_row = 1,
      na.strings = ""
    ) %>%
    openxlsx2::wb_set_cell_style(
      dims = openxlsx2::wb_dims(x = tabla_attr_rodal_final, select = "col_names"),
      style = wb_ap5$styles_mgr$get_xf_id("header_cellxfs")
    ) %>%
    openxlsx2::wb_add_border(
      dims = openxlsx2::wb_dims(x = tabla_attr_rodal_final, select = "data"),
      inner_hgrid = "thin",
      inner_vgrid = "thin"
    ) %>%
  ...
  ft_corta <- df_corta %>%
    ...
    `names<-`(
      c(
        "Predio N°",
        "Área a intervenir_N°",
        "Área a reforestar_Superficie (ha)",
        "Año",
        "Clase Capac. Uso",
  ...
  for (i in seq_len(nrow(stf))) {
    ft_corta <- flextable::footnote(
      ft_corta,
      part = 'body',
      i = c(which( df_corta$Subtipo_fo == stf$Subtipo_fo[i])),
      j = 6,
      value = flextable::as_paragraph(stf$Subtipo_fo[i]),
      ref_symbols = paste(rep('*', i), collapse = "")
    )
  ...
  wb_ap5 <- wb_ap5 %>%
    ...
    flexlsx::wb_add_flextable(sheet = "Corta", ft = ft_corta)
  ...
  tbl_resumen <- tabla_areas %>%
    ...
    `names<-`(
      c(
        ...
        "Corta_N° predios",
        "Corta_Superficie (ha)",
        "Reforestación_N° predios",
        "Reforestación_Superficie (ha)"
      )
    ) %>%
    flextable::flextable() %>%
    flextable::separate_header(split = "_") %>%
  ...
  if (isTruthy(areas) & isTruthy(obras)) {
    tbl_obras <- areas %>%
      ...
      flextable::merge_v(j = 1) %>%
      flextable::merge_h_range(
        i = ~ `Temporalidad de la obra` == "Total",
        j1 = "Temporalidad de la obra",
        j2 = "Obra",
        part = "body"
      ) %>%
      flextable::theme_box() %>%
  ...
  tbl_est <- bd_flora %>%
    ...
    dplyr::summarise(
      ...
      cuasivarianza = ((1 - (
        n * (500 / 10000) / (rodales$Sup_ha %>% sum())
      )) * (stats::sd(Nha) ^ 2 / n)) %>% janitor::round_half_up(2),
      ...
      Int_conf = paste0(
        janitor::round_half_up(Promedio - E_abs) %>% mark(),
        " - ",
        janitor::round_half_up(Promedio + E_abs) %>% mark()
      )
    ) %>%
...
apendice_5_PAS151 <- function(
  ...
  wb_ap5 <- wb_ap5 %>%
    ...
    openxlsx2::wb_set_cell_style(
      dims = openxlsx2::wb_dims(x = tbl_sup_predios, select = "col_names"),
      style = wb_ap5$styles_mgr$get_xf_id("header_cellxfs")
    ) %>%
    openxlsx2::wb_add_border(
      dims = openxlsx2::wb_dims(x = tbl_sup_predios, select = "data"),
      inner_hgrid = "thin",
      inner_vgrid = "thin"
    ) %>%
    openxlsx2::wb_add_numfmt(
      dims = openxlsx2::wb_dims(x = tbl_sup_predios, cols = 4, select = "data"),
      numfmt = "#,##0.00"
    ) %>%
  ...
  nha_otros <- tabla_areas %>%
    ...
    dplyr::mutate(
      ...
      cob_x_sp = purrr::map2(Parcelas, nha_x_sp, function(x, y) {
        cob_x_sp_fun(parcelas = x, bd_flora = bd_flora, cov_as_range = cov_as_range, cov_fp = cov_fp) %>%
    ...
    tidyr::unnest(Parcelas) %>%
    dplyr::select(-c(dplyr::matches("nha_x_sp|cob_x_sp"))) %>%
    suppressMessages() %>% suppressWarnings()
  ...
  tbl_nha_cob <- dplyr::bind_rows(
    nha_ptos, nha_est, nha_otros %>% dplyr::select(-c(Tipo_veg, Parcelas))
  ) %>%
    dplyr::mutate_at("Cob_ind", stringi::stri_replace_all_regex, "\\.", ",") %>%
  ...
  for (i in seq_along(habitos)) {
    tbl_nha <- tbl_nha_cob %>%
      ...
      tidyr::pivot_wider(
        names_from = "Habito",
        values_from = c("Nha", "Cobertura"),
        names_glue = "{Habito}_{.value}"
      ) %>%
      dplyr::arrange(N_Predio, N_Area, Especie) %>%
      dplyr::rename("Predio N°" = 1, "N° Sector" = 2) %>%
      dplyr::rename_at("Especie", ~ paste0(habitos[i], "_", .)) %>%
      dplyr::rename_at(
        dplyr::vars(dplyr::contains("Nha")),
        stringi::stri_replace_all_regex,
        "Nha",
        "Densidad (Ind/ha)"
      ) %>%
      dplyr::rename_at(
        dplyr::vars(dplyr::contains("Cobertura")),
        stringi::stri_replace_all_regex,
        "Cobertura",
        "Cobertura (%)"
      ) %>%
  ...
  tbl_corta <- tabla_areas %>%
    dplyr::mutate(
      Ano = "-",
      Tipo = "Corta y descepado",
      Descripcion = "Extracción de ejemplares para instalación de obras",
      Cob_final = 0
    ) %>%
  ...
  tbl_est <- bd_flora %>%
    ...
    dplyr::summarise(
      ...
      E_rel = ((E_abs / Promedio) * 100) %>% janitor::round_half_up(1),
      Int_conf = paste0(
        janitor::round_half_up(Promedio - E_abs),
        " - ",
        janitor::round_half_up(Promedio + E_abs)
      )
    ) %>%
...
#' @rdname get_appendices
#' @export
apendice_5_PAS151_nuevo <- function(
    bd_flora,
    rodales,
    tabla_predios,
    tabla_areas,
  ...
  wb_ap5 <- wb_ap5 %>%
    openxlsx2::wb_add_worksheet("1.3.1") %>%
    openxlsx2::wb_add_data(
      sheet = "1.3.1",
      x = tbl_predios,
      start_col = 1,
  ...
  wb_ap5 <- wb_ap5 %>%
    openxlsx2::wb_add_worksheet("1.3.2") %>%
    openxlsx2::wb_add_data(
      sheet = "1.3.2",
      x = tbl_sup_predios,
      start_col = 1,
  ...
  tbl_areas <- tabla_areas %>%
    dplyr::mutate(SG_SI = "", SG_NO = "", Dist_Area_Prot = "") %>%
    dplyr::select(N_Predio, N_Area, Sup_ha, Ran_Pend, SG_SI, SG_NO, Clase_Eros) %>%
    `names<-`(
      c(
        "Predio N°",
        "N° Sector",
  ...
  wb_ap5 <- wb_ap5 %>%
    openxlsx2::wb_add_worksheet("3.1") %>%
    flexlsx::wb_add_flextable(
      sheet = "3.1",
      ft = tbl_areas
    )
  ...
  if (any(c("ID_Hidro", "Tipo_Dren", "Tipo_Perma", "Nombre_curso") %in% names(tabla_areas))) {
    tbl_hidro <- tabla_areas %>%
      dplyr::mutate(Ancho = "") %>%
      dplyr::select(ID_Hidro, N_Area, Sup_ha, Distancia, Tipo_Dren, Tipo_Perma, Ancho) %>%
      `names<-`(
        c(
          "ID",
          "Área N°",
          "Superficie (ha)",
          "Distancia a los sectores a intervenir (m)",
          "Tipo drenaje",
          "Temporalidad",
          "Rango de sección del cauce (m2)"
        )
      ) %>%
      flextable::flextable() %>%
      flextable::merge_v(j = 1) %>%
    ...
    wb_ap5 <- wb_ap5 %>%
      openxlsx2::wb_add_worksheet("3.2", grid_lines = F) %>%
      flexlsx::wb_add_flextable(
        sheet = "3.2",
        ft = tbl_hidro
      )
      ...
      nha_otros %>%
        ...
        dplyr::bind_rows(
          parc_x_tipo %>%
            ...
            dplyr::inner_join(
              tabla_attr_rodal %>% dplyr::filter(Nom_attr == "Estimación por Tipo vegetacional") %>% dplyr::select(Tipo_veg, N_Rodal)
            ) %>%
            dplyr::select(-Tipo_veg)
        )
    ) %>%
  ...
  tbl_nha_cob <-
    dplyr::bind_rows(nha_ptos, nha_est, nha_otros %>% dplyr::select(-c(Tipo_veg, Parcelas))) %>%
    dplyr::select(-N_Rodal) %>%
    dplyr::arrange(N_Predio, N_Area, Nha, Cob_ind) %>%
    dplyr::rename(Cobertura = Cob_ind)

  tabla_comp_1 <- tbl_nha_cob %>%
    dplyr::count(Especie, Habito) %>%
    dplyr::select(-n) %>%
    left_join(bd_flora %>% dplyr::count(Especie, Habito, DS_68, RCE) %>% dplyr::select(-n)) %>%
    dplyr::select(DS_68, Especie, Habito) %>%
    ...
    `names<-`(
      c(
        ...
        "Especie",
        "Hábito",
        "Dispone de métodos de propagación documentados (si/no)",
        "Fuente de información que documenta método de propagación"
      )
    ) %>%
    flextable::flextable() %>%
    flextable::merge_v(j = c(1)) %>%
    flextable::italic(j = 2, italic = T) %>%
    flextable::autofit() %>%
    flextable::theme_box() %>%
    flextable::valign(part = "header", valign = "center") %>%
    flextable::align(part = "header", align = "center") %>%
  ...
  tabla_comp_2 <- tbl_nha_cob %>%
    ...
    `names<-`(
      c(
        ...
        "Densidad (ind/ha)",
        "Cobertura (%)",
        "Densidad Total (ind/ha)"
      )
    ) %>%
    flextable::flextable() %>%
    flextable::merge_v(j = 1) %>%
    flextable::merge_v(j = 2, target = c(2, 7)) %>%
    flextable::italic(j = 3, italic = T) %>%
    flextable::autofit() %>%
    flextable::theme_box() %>%
    flextable::valign(part = "header", valign = "center") %>%
    flextable::align(part = "header", align = "center") %>%
  ...
  wb_ap5 <- wb_ap5 %>%
    openxlsx2::wb_add_worksheet("3.4.1") %>%
    flexlsx::wb_add_flextable(
      sheet = "3.4.1",
      ft = tabla_comp_1
    ) %>%
  ...
  tabla_ECC <- tbl_nha_cob %>%
    ...
    `names<-`(
      c(
        ...
        "Sector N°",
        "Especie",
        "En nómina de especies (si/no)",
        "Valor_Singular/Representativa",
        "Valor_Categoría de conservación",
        "Valor_N° de Decreto MMA"
      )
    ) %>%
    flextable::flextable() %>%
    flextable::separate_header(split = "_") %>%
  ...
  wb_ap5 <- wb_ap5 %>%
    openxlsx2::wb_add_worksheet("3.4.2") %>%
    flexlsx::wb_add_flextable(
      sheet = "3.4.2",
      ft = tabla_ECC,
      start_col = 7,
  ...
  if (isTruthy(areas) & isTruthy(obras)) {
    inter_x_temp <- areas %>%
      sf::st_intersection(obras %>% dplyr::select(Tipo, Tipo_obra, Nom_obra, Fase)) %>%
      split(.$Tipo) %>%
      purrr::map(function(x) {
        x %>%
          dplyr::count(Tipo_Obra, Nom_Obra, Fase, N_Predio, N_Area) %>%
          dplyr::select(-n) %>%
          dplyr::mutate(
            Sup_ha = sf::st_area(geometry) %>% units::set_units(ha) %>% units::drop_units() %>% janitor::round_half_up(2),
            Inter_i = "-",
          ...
          sf::st_drop_geometry() %>%
          dplyr::arrange(N_Predio, N_Area, Tipo_Obra, Nom_Obra) %>%
          `names<-`(
            c(
              "Tipo de obra",
              "Actividad",
              ...
              "Sector(es) N°",
              "Superficie a intervenir (ha)",
              "Año de intervención_Inicio",
              "Año de intervención_Término"
            )
          ) %>%
          flextable::flextable() %>%
          flextable::separate_header(split = "_") %>%
    ...
    for (i in sort(names(inter_x_temp), decreasing = T) %>% paste0("Obras_", .)) {
      wb_ap5 <- wb_ap5 %>%
        openxlsx2::wb_add_worksheet(sheet = i) %>%
        flexlsx::wb_add_flextable(
          sheet = i,
          ft = inter_x_temp[i]
        )
