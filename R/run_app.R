...
#' @importFrom bsplus bs_accordion bs_append bs_attach_modal bs_carousel bs_carousel_image bs_embed_tooltip bs_modal bs_modal_closebutton bs_set_data bs_set_opts shiny_iconlink shinyInput_label_embed use_bs_popover use_bs_tooltip
#' @importFrom config get
#' @importFrom dplyr across add_count anti_join arrange bind_cols bind_rows case_match case_when contains count cur_group_id desc ends_with everything expr filter group_by inner_join last_col left_join matches mutate mutate_all mutate_at mutate_if n pull relocate rename rename_all rename_at rename_if sample_n select slice slice_min slice_sample starts_with summarise summarise_at sym syms tally ungroup vars
...
#' @importFrom openxlsx2 create_border create_cell_style create_font read_xlsx wb_add_border wb_add_cell_style wb_add_data wb_add_fill wb_add_font wb_add_image wb_add_worksheet wb_color wb_dims wb_merge_cells wb_page_setup wb_save wb_set_cell_style wb_set_col_widths wb_workbook write_xlsx
...
#' @importFrom stringi stri_cmp_equiv stri_count_regex stri_detect_regex stri_extract stri_extract_all_regex stri_length stri_locate_all_regex stri_pad_left stri_replace_all_regex stri_split_regex stri_sub stri_trans_general stri_trans_tolower stri_trans_totitle stri_trans_toupper stri_trim stri_replace_all_fixed
#' @importFrom terra as.contour crop crs `crs<-` extract minmax project rast terrain
